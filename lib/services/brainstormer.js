...
OUTPUT STRICT JSON with fields:
{
  ...
  "reason": string (brief rationale)
        ...
        const response = await (0, ai_client_1.getAIClient)().analyzePrompt(originalPrompt, 
...
OUTPUT JSON ONLY with field: { "rawPrompt": string }`;
        const originalPrompt = `INITIAL_IDEA:\n${initialIdea}\n\nTRANSCRIPT:\n${transcript
            .map((qa, i) => `${i + 1}. Q: ${qa.question}\n   A: ${qa.answer}`)
            .join('\n') || '(none)'}\n\nPROMPT_TYPE: ${promptType}`;
        const response = await (0, ai_client_1.getAIClient)().analyzePrompt(originalPrompt, 'reasoning', modelType, systemPrompt);
        const parsed = (0, json_parser_1.parseAIResponse)(response);
