...
exports.brainstormer = exports.Brainstormer = void 0;
const ai_client_factory_1 = require("./ai-client-factory");
const json_parser_1 = require("../utils/json-parser");
class Brainstormer {
    async generateNextQuestions(initialIdea, transcript, promptType, modelType) {
...
OUTPUT STRICT JSON with fields:
{
  ...
  "reason": string (brief rationale)
        ...
        const response = await ai_client_factory_1.AIClientFactory.analyzePrompt(originalPrompt, 
        // Treat this as reasoning regardless, since it's interviewing
        'reasoning', modelType, systemPrompt);
        try {
            const parsed = (0, json_parser_1.parseAIResponse)(response);
    ...
    async synthesizeRawPrompt(initialIdea, transcript, promptType, modelType) {
...
OUTPUT JSON ONLY with field: { "rawPrompt": string }`;
        const originalPrompt = `INITIAL_IDEA:\n${initialIdea}\n\nTRANSCRIPT:\n${transcript
            .map((qa, i) => `${i + 1}. Q: ${qa.question}\n   A: ${qa.answer}`)
            .join('\n') || '(none)'}\n\nPROMPT_TYPE: ${promptType}`;
        const response = await ai_client_factory_1.AIClientFactory.analyzePrompt(originalPrompt, 'reasoning', modelType, systemPrompt);
        const parsed = (0, json_parser_1.parseAIResponse)(response);
...
exports.Brainstormer = Brainstormer;
exports.brainstormer = new Brainstormer();
