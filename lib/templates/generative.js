...
exports.GENERATIVE_ANALYSIS_PROMPT = exports.GENERATIVE_COMPACT_ANALYSIS_PROMPT = exports.GENERATIVE_DETAILED_ANALYSIS_PROMPT = void 0;
exports.buildGenerativeTemplate = buildGenerativeTemplate;
function buildGenerativeTemplate(data, format, flavor = 'detailed') {
    if (flavor === 'compact') {
        return buildGenerativeCompactMarkdown(data);
    }
    if (format === 'json') {
...
function buildGenerativeMarkdown(data) {
...
function buildGenerativeJSON(data) {
    const isDetailed = 'subcategories' in data;
    if (isDetailed) {
        return buildGenerativeDetailedJSON(data);
    }
    else {
        return buildGenerativeCompactJSON(data);
    }
}
function buildGenerativeDetailedJSON(data) {
    const jsonStructure = {
...
function buildGenerativeCompactJSON(data) {
    const jsonStructure = {
...
function buildGenerativeCompactMarkdown(data) {
    const stripLeadingListMarker = (text) => {
        return text.replace(/^\s*(?:\d+[\.)]|[-*•])\s+/, '');
    };
    let compact = `# Persona
...
exports.GENERATIVE_DETAILED_ANALYSIS_PROMPT = `You are an expert AI prompt engineer. Your task is to analyze an unstructured prompt and transform it into a comprehensive, well-structured generative prompt optimized for AI model-to-model communication for building features and products.
...
exports.GENERATIVE_COMPACT_ANALYSIS_PROMPT = `You are an expert AI prompt engineer. Your task is to analyze an unstructured prompt and transform it into a focused, compact generative prompt optimized for quick, specific AI model-to-model communication for targeted development tasks.
...
exports.GENERATIVE_ANALYSIS_PROMPT = exports.GENERATIVE_DETAILED_ANALYSIS_PROMPT;
